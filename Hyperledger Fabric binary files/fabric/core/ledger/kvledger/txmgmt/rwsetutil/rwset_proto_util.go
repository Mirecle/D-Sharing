...
import (
	...
	//"github.com/hyperledger/fabric/core/ledger/kvledger/txmgmt/mproof"
)
...
/*
	fmt.Println("Yang 2023 11-24 getstate_Read")
	fmt.Println("key is :%v",key)
	fmt.Println("version is :%v",version)
	if key == "mproof"{
		respvk := mproof.Makepvk()
		if respvk {
		}
		_, respro := mproof.Makeproof()
		if respro {
		}
		resver := mproof.Makeverify()
		if resver {
			fmt.Println("验证成功")
			key = "mprooft"
			return &kvrwset.KVRead{Key: key, Version: newProtoVersion(version)}
